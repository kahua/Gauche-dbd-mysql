...
(test* "mysql-fetch-field" <mysql-field> (mysql-fetch-field *result*) (lambda (class obj)
									(eq? class (class-of obj))))
(test* "mysql-fetch-field" <mysql-field> (mysql-fetch-field *result*) (lambda (class obj)
									(eq? class (class-of obj))))
(test* "mysql-fetch-field" #f (mysql-fetch-field *result*))
...
(let ((field0 (mysql-fetch-field-direct *result* 0))
  ...
  (for-each (lambda (args)
	    ...
	    `((name ,string-ci=? "ID" "DATA")
	      ...
	      (db ,(lambda (expected value) (if value
						(string-ci=? expected value)
						#t))
		  "test" "test")
